...
export default function EditApplicantPage() {
  ...
  const {
    ...
    resolver: zodResolver(applicantSchema) as any,  // ✅ Added 'as any' to fix type
    defaultValues: {
      total_experience: 0,
  ...
  // Reset form when applicant data loads
  if (applicant && !isLoading) {
    reset({
      ...
      alternate_phone: applicant.alternatePhone || '',
      gender: applicant.gender || '',
      date_of_birth: applicant.dateOfBirth?.split('T')[0] || '',
      current_city: applicant.currentCity || '',
      current_state: applicant.currentState || '',
      current_country: applicant.currentCountry || '',
      address: applicant.address || '',
      highest_qualification: applicant.highestQualification || '',
      specialization: applicant.specialization || '',
      university: applicant.university || '',
      graduation_year: applicant.graduationYear || undefined,
      total_experience: applicant.totalExperience || 0,
      current_company: applicant.currentCompany || '',
      current_designation: applicant.currentDesignation || '',
      current_ctc: applicant.currentCTC || undefined,
      expected_ctc: applicant.expectedCTC || undefined,
      notice_period: applicant.noticePeriod || undefined,
      ...
      application_source: applicant.applicationSource,
      application_date: applicant.applicationDate?.split('T')[0] || '',
  ...
  const updateMutation = useMutation({
    mutationFn: (data: ApplicantFormData) => applicantService.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['applicant', id] });
      queryClient.invalidateQueries({ queryKey: ['applicants'] });
      toast.success('Applicant updated successfully');
      router.push(`/recruitment/applicants/${id}`);
    },
  });
