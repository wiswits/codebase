...
import { FaSignOutAlt, FaMapMarkerAlt, FaClock, FaHome } from 'react-icons/fa';
...
const CheckOut = ({ onCheckOut }) => {
  ...
  const handleCheckOut = async () => {
    // Validate notes
    if (!notes.trim()) {
      toast.warning('Please add notes about your check-out');
      return;
    }
    ...
    try {
      const data = {
        ...
        notes,
        isWFH,
        checkOutType: isWFH ? 'wfh' : 'office'
      };
      const response = await attendanceApi.checkOut(data);
      toast.success(isWFH ? '✅ Work from home check-out successful!' : '✅ Check-out successful!');
      if (onCheckOut) onCheckOut(response.data);
    } catch (error) {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center space-x-2 p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg">
          ...
          />
          <label htmlFor="wfhCheckOut" className="flex items-center space-x-2 text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={isWFH ? "Add notes about your work from home check-out..." : "Add notes about your check-out..."}
        ...
        <button
          ...
          className={`w-full flex items-center justify-center space-x-2 px-4 py-3 rounded-lg text-white font-medium transition-colors ${
            loading || (!isWFH && !location) || (!isWFH && location && !isWithinRadius)
              ...
              : isWFH ? 'bg-purple-600 hover:bg-purple-700' : 'bg-red-600 hover:bg-red-700'
          }`}
        >
          {loading ? (
            <>
            ...
            <>
              {isWFH ? <FaHome /> : <FaSignOutAlt />}
              <span>{isWFH ? 'Check Out (WFH)' : 'Check Out'}</span>
            </>
          )}
        </button>
      </div>
