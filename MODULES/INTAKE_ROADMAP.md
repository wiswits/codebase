...
1. Read `MODULES/NN-<Module>/STATUS.md` (its Intake table says what is in the way), then
   `intake/<Build>/STATUS.md` — confirm the verdict still holds (the native side may have
   changed since it was written).
2. Diff `intake/<Build>/PRD/` against `intake/<Build>/code/` — build what was actually
