...
export default function EditVacancyPage() {
  ...
  // Reset form when vacancy data loads
  if (vacancy && !isLoading) {
    reset({
      vacancy_code: vacancy.vacancyCode,
      ...
      number_of_openings: vacancy.numberOfOpenings,
      experience_required: vacancy.experienceRequired || '',
      ...
      preferred_skills: vacancy.preferredSkills || '',
      education_required: vacancy.educationRequired || '',
      application_start_date: vacancy.applicationStartDate?.split('T')[0] || '',
      application_end_date: vacancy.applicationEndDate?.split('T')[0] || '',
      expected_joining_date: vacancy.expectedJoiningDate?.split('T')[0] || '',
  ...
  const updateMutation = useMutation({
    mutationFn: (data: VacancyFormData) => vacancyService.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['vacancy', id] });
      queryClient.invalidateQueries({ queryKey: ['vacancies'] });
      toast.success('Vacancy updated successfully');
      router.push(`/recruitment/vacancies/${id}`);
    },
  });
  ...
  return (
    <div>
      <PageHeader
        title="Edit Vacancy"
        description="Update job vacancy details"
        actions={
          <Button
            variant="outline"
            onClick={() => router.back()}
