...
module.exports = function(pool) {
    ...
    // ============================================
    router.post('/', auth.isStudent, async (req, res, next) => {
        try {
            ...
            // Set SLA deadlines (1 hour for acknowledge, 24 hours for resolve)
    ...
    // ============================================
    // GET DOUBTS (Student - own doubts)
    // ============================================
    router.get('/my', auth.isStudent, async (req, res, next) => {
        try {
            const studentId = req.userId;
            ...
            const result = await pool.query(
                `SELECT d.*, 
                 ...
                 WHERE d.student_id = (SELECT id FROM students WHERE user_id = $1)
    ...
    // ============================================
    // GET DOUBT QUEUE (Faculty)
    // ============================================
    router.get('/queue', auth.isFaculty, async (req, res, next) => {
        try {
            ...
            // Order by SLA urgency (deadline approaching)
    ...
    // ============================================
    // ASSIGN DOUBT (Faculty)
    // ============================================
    router.put('/:id/assign', auth.isFaculty, async (req, res, next) => {
        try {
            const { id } = req.params;
    ...
    // ============================================
    // RESOLVE DOUBT (Faculty)
    // ============================================
    router.put('/:id/resolve', auth.isFaculty, async (req, res, next) => {
        try {
            const { id } = req.params;
            ...
            if (resolution_message) {
                await pool.query(
                    `INSERT INTO doubt_replies (doubt_id, user_id, message, is_faculty_response)
                     VALUES ($1, $2, $3, true)`,
                    [id, req.userId, resolution_message]
                );
            }

            // Send notification to student (to be implemented)
    ...
    router.post('/:id/reply', async (req, res, next) => {
        try {
            ...
            const isFaculty = req.userRole === 'faculty' || req.userRole === 'admin';

            const result = await pool.query(
                `INSERT INTO doubt_replies (doubt_id, user_id, message, file_url, is_faculty_response)
                 VALUES ($1, $2, $3, $4, $5)
                 RETURNING *`,
                [id, req.userId, message, file_url, isFaculty]
            );

            // If faculty replied, update doubt status
            if (isFaculty) {
                await pool.query(
                    `UPDATE doubts SET updated_at = CURRENT_TIMESTAMP WHERE id = $1`,
                    [id]
                );
            }
    ...
    // ============================================
    // RATE DOUBT RESOLUTION (Student)
    // ============================================
    router.put('/:id/rate', auth.isStudent, async (req, res, next) => {
        try {
            const { id } = req.params;
    ...
    // ============================================
    // GET SLA STATS (Faculty/Admin)
    // ============================================
    router.get('/sla-stats', auth.isFaculty, async (req, res, next) => {
        try {
            const result = await pool.query(
                `SELECT 
